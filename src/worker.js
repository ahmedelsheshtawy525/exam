...
function credentialPage(c,request){
  ...
  const details=`<section class="details"><div class="detail-main"><div class="detail-head"><div><div class="kicker left">Credential details</div><h2>Verification record</h2></div><span class="status ${status?'':'revoked'}">● ${status?'VALID':'REVOKED'}</span></div><div class="detail-grid"><div><span>Type</span><b>${htmlEscape(c.type||'Training')}</b></div><div><span>Level</span><b>${htmlEscape(c.level||'Intermediate')}</b></div><div><span>Format</span><b>${htmlEscape(c.format||'Online')}</b></div><div><span>Duration</span><b>${htmlEscape(c.duration||'')}</b></div><div><span>Issued</span><b>${htmlEscape(issued)}</b></div><div class="certificate-id-field"><span>Certificate ID</span><div class="certificate-id-row"><b class="copyable-id" title="Select or copy certificate ID">${htmlEscape(c.certificate_number)}</b><button type="button" class="copy-id-btn" onclick="copyCertificateId(this,${JSON.stringify(String(c.certificate_number))})" aria-label="Copy certificate ID" title="Copy certificate ID">Copy</button></div></div></div>${description?`<div class="description-box"><span>Assessment description</span><p>${htmlEscape(description)}</p></div>`:''}<div class="content-block"><div class="kicker left">Skills</div><div class="skills">${content}</div></div></div><aside class="source-panel"><div class="kicker left">Source</div><h3>Credential issuer</h3><div class="source-row"><span>Issued by</span><b>${htmlEscape(c.issued_by||'Ahmed Elsheshtawy')}</b></div><div class="source-row"><span>Assessment</span><b>${htmlEscape(c.exam_title||'Assessment')}</b></div><div class="source-row"><span>Verification</span><b>Official certificate record</b></div><a class="source-link" href="${sourceUrl}" target="_blank" rel="noopener">Open source ↗</a></aside></section>`;

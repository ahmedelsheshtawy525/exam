...
async function api(request,env,ctx){
  ...
  if(adminSession(s)){
    ...
    if(m==='DELETE'&&p.match(/^\/api\/admin\/exams\/(\d+)\/?$/)){const id=idNum((p.match(/^\/api\/admin\/exams\/(\d+)/)||[])[1]);if(!id)return bad('Invalid exam');const exists=await env.DB.prepare('SELECT id FROM exams WHERE id=?').bind(id).first();if(!exists)return bad('Exam not found',404);await env.DB.batch([env.DB.prepare('DELETE FROM certificates WHERE exam_id=?').bind(id),env.DB.prepare('DELETE FROM answers WHERE attempt_id IN (SELECT id FROM exam_attempts WHERE exam_id=?)').bind(id),env.DB.prepare('DELETE FROM results WHERE exam_id=?').bind(id),env.DB.prepare('DELETE FROM exam_attempts WHERE exam_id=?').bind(id),env.DB.prepare('DELETE FROM questions WHERE exam_id=?').bind(id),env.DB.prepare('DELETE FROM exams WHERE id=?').bind(id)]);return json({ok:true})}

...
function credentialPage(c,request){const origin=new URL(request.url).origin;const verify=`${origin}/verify/${encodeURIComponent(c.certificate_number)}`;const skills=parseSkills(c.skills);const status=c.status==='valid';const issued=new Date(String(c.issued_at).includes('T')?c.issued_at:c.issued_at+'Z').toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'});return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="description" content="Verified digital credential issued by Ahmed Elsheshtawy"><title>${htmlEscape(c.title)} — ${htmlEscape(c.student_name)}</title><style>
:root{--ink:#101828;--muted:#667085;--line:#e4e7ec;--soft:#f8fafc;--accent:#f97316;--success:#12b76a}*{box-sizing:border-box}body{margin:0;background:#f7f8fa;color:var(--ink);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.page{max-width:1120px;margin:0 auto;padding:28px 22px 70px}.topbar{height:58px;display:flex;align-items:center;justify-content:space-between;margin-bottom:22px}.brand{display:flex;align-items:center;gap:11px;font-weight:800}.brand img{width:34px;height:34px;object-fit:contain}.brand small{display:block;color:var(--muted);font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase}.actions{display:flex;gap:9px;flex-wrap:wrap}.btn{border:1px solid var(--line);background:#fff;color:var(--ink);border-radius:10px;padding:10px 14px;font-weight:700;font-size:13px;cursor:pointer;text-decoration:none}.btn.primary{background:#101828;color:#fff;border-color:#101828}.btn:hover{transform:translateY(-1px)}.layout{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(300px,.75fr);gap:22px}.credential,.sidecard{background:#fff;border:1px solid var(--line);border-radius:18px;box-shadow:0 10px 35px rgba(16,24,40,.06)}.credential{overflow:hidden}.cover{padding:38px 40px 34px;background:linear-gradient(135deg,#fff 0%,#fff8f1 100%);border-bottom:1px solid var(--line);position:relative}.cover:after{content:"";position:absolute;right:-80px;top:-100px;width:260px;height:260px;border:1px solid #f0e2d4;border-radius:50%}.issuer{font-size:12px;color:var(--muted);font-weight:700;letter-spacing:.12em;text-transform:uppercase}.cover h1{font-size:42px;line-height:1.08;margin:16px 0 20px;max-width:720px;letter-spacing:-.035em}.issuedto{color:var(--muted);font-size:13px;margin-bottom:7px}.student{font-size:28px;font-weight:800;letter-spacing:-.02em}.completion{margin-top:13px;color:#475467;font-size:15px;line-height:1.6}.completion b{color:var(--ink)}.score{display:inline-flex;align-items:center;gap:8px;margin-top:18px;padding:9px 13px;border-radius:999px;background:#fff;border:1px solid #f0d7c3;font-weight:800}.body{padding:30px 40px}.section-title{font-size:12px;font-weight:800;color:var(--muted);letter-spacing:.12em;text-transform:uppercase;margin-bottom:15px}.details{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));border-top:1px solid var(--line);border-left:1px solid var(--line)}.detail{padding:17px 18px;border-right:1px solid var(--line);border-bottom:1px solid var(--line)}.label{font-size:11px;color:var(--muted);font-weight:700;letter-spacing:.09em;text-transform:uppercase}.value{margin-top:6px;font-size:15px;font-weight:750}.description{margin-top:30px;color:#475467;line-height:1.75;font-size:15px}.skills{display:flex;gap:8px;flex-wrap:wrap;margin-top:16px}.skill{padding:8px 12px;border:1px solid var(--line);border-radius:999px;background:var(--soft);font-size:13px;font-weight:650}.side{display:flex;flex-direction:column;gap:22px}.sidecard{padding:25px}.verify-head{display:flex;justify-content:space-between;align-items:center;gap:12px}.verified{display:inline-flex;align-items:center;gap:7px;padding:8px 11px;border-radius:999px;background:#ecfdf3;color:#027a48;font-size:12px;font-weight:800}.verified.revoked{background:#fef3f2;color:#b42318}.dot{width:7px;height:7px;border-radius:50%;background:currentColor}.sidecard h2{font-size:20px;margin:14px 0 9px}.sidecard p{color:var(--muted);font-size:14px;line-height:1.6}.idbox{padding:13px;border:1px solid var(--line);background:var(--soft);border-radius:11px;font:12px ui-monospace,SFMono-Regular,Menlo,monospace;word-break:break-all;margin-top:15px}.copy{width:100%;margin-top:10px}.issuerbox{display:flex;align-items:center;gap:13px}.issuerbox img{width:46px;height:46px;object-fit:contain;border:1px solid var(--line);border-radius:12px;padding:7px}.issuerbox b{display:block}.issuerbox span{display:block;color:var(--muted);font-size:12px;margin-top:3px}.footer{margin-top:24px;text-align:center;color:#98a2b3;font-size:12px}.print-only{display:none}@media(max-width:850px){.layout{grid-template-columns:1fr}.cover h1{font-size:34px}}@media(max-width:600px){.page{padding:12px 12px 45px}.topbar{height:auto;align-items:flex-start;gap:12px}.actions{justify-content:flex-end}.actions .btn{padding:8px 10px}.cover{padding:28px 24px}.body{padding:25px 24px}.cover h1{font-size:29px}.student{font-size:23px}.details{grid-template-columns:1fr}.sidecard{padding:21px}}@media print{body{background:#fff}.page{max-width:none;padding:0}.topbar,.actions,.footer{display:none}.layout{display:block}.side{display:none}.credential{box-shadow:none;border:1px solid #ddd}.cover{padding:35px}.body{padding:30px}.print-only{display:block}}
</style></head><body><main class="page"><header class="topbar"><div class="brand"><img src="/ae-logo.png" alt="Ahmed Elsheshtawy"><div>Ahmed Elsheshtawy<small>Digital Credential</small></div></div><div class="actions"><button class="btn" onclick="shareAward()">Share Your Award</button><button class="btn" onclick="window.print()">Download / Print</button><button class="btn primary" onclick="copyLink()">Copy Link</button></div></header><div class="layout"><section class="credential"><div class="cover"><div class="issuer">Credential issued by ${htmlEscape(c.issued_by||'Ahmed Elsheshtawy')}</div><h1>${htmlEscape(c.title)}</h1><div class="issuedto">Issued to</div><div class="student">${htmlEscape(c.student_name)}</div><div class="completion">Successfully completed <b>${htmlEscape(c.exam_title)}</b> and met the required assessment criteria.</div><div class="score">✓ ${Number(c.percentage).toFixed(1)}% Score</div></div><div class="body"><div class="section-title">Credential Details</div><div class="details"><div class="detail"><div class="label">Issue date</div><div class="value">${htmlEscape(issued)}</div></div><div class="detail"><div class="label">Issued by</div><div class="value">${htmlEscape(c.issued_by||'Ahmed Elsheshtawy')}</div></div><div class="detail"><div class="label">Type</div><div class="value">${htmlEscape(c.type||'Training')}</div></div><div class="detail"><div class="label">Level</div><div class="value">${htmlEscape(c.level||'Intermediate')}</div></div><div class="detail"><div class="label">Format</div><div class="value">${htmlEscape(c.format||'Online')}</div></div><div class="detail"><div class="label">Duration</div><div class="value">${htmlEscape(c.duration||'—')}</div></div><div class="detail"><div class="label">Score</div><div class="value">${Number(c.score)} / ${Number(c.score)+Math.max(0,Number(c.total_points||c.score)-Number(c.score)) || '—'} (${Number(c.percentage).toFixed(1)}%)</div></div><div class="detail"><div class="label">Status</div><div class="value">${status?'Valid':'Revoked'}</div></div></div><div class="description"><div class="section-title">Description</div>${htmlEscape(c.description||'This credential confirms successful completion of the assessment.')}</div><div style="margin-top:28px"><div class="section-title">Skills</div><div class="skills">${skills.map(x=>`<span class="skill">${htmlEscape(x)}</span>`).join('')||'<span style="color:#667085">No skills listed.</span>'}</div></div></div></section><aside class="side"><section class="sidecard"><div class="verify-head"><div class="section-title" style="margin:0">Credential Verification</div><span class="verified ${status?'':'revoked'}"><span class="dot"></span>${status?'Valid':'Revoked'}</span></div><h2>${status?'Verified Credential':'Revoked Credential'}</h2><p>This credential can be independently verified using its unique certificate number.</p><div class="idbox">${htmlEscape(c.certificate_number)}</div><button class="btn copy" onclick="copyLink()">Copy Verification Link</button></section><section class="sidecard"><div class="section-title">Distributed by</div><div class="issuerbox"><img src="/ae-logo.png" alt="Ahmed Elsheshtawy"><div><b>${htmlEscape(c.issued_by||'Ahmed Elsheshtawy')}</b><span>Training &amp; Professional Credentials</span></div></div></section><section class="sidecard"><div class="section-title">Verification URL</div><p>Anyone with this link can verify the credential without signing in.</p><div class="idbox">${htmlEscape(verify)}</div></section></aside></div><div class="footer">This credential is digitally issued and verified by Ahmed Elsheshtawy.</div></main><script>const verifyUrl=${JSON.stringify(verify)};async function copyLink(){try{await navigator.clipboard.writeText(verifyUrl);alert('Verification link copied.')}catch(e){prompt('Copy verification link:',verifyUrl)}}async function shareAward(){if(navigator.share){try{await navigator.share({title:${JSON.stringify(c.title)},text:${JSON.stringify('Verified credential issued to '+String(c.student_name||''))},url:verifyUrl})}catch(e){}}else{copyLink()}}</script></body></html>`}

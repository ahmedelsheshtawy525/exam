...
function credentialPage(c,request){
  ...
  const details=`<section class="details"><div class="detail-head"><div><div class="kicker left">Certificate details</div><h2>Achievement overview</h2></div><span class="status ${status?'':'revoked'}">● ${status?'VALID':'REVOKED'}</span></div>${description?`<p class="description">${htmlEscape(description)}</p>`:''}<div class="detail-grid"><div><span>Exam</span><b>${htmlEscape(c.exam_title||'Assessment')}</b></div><div><span>Type</span><b>${htmlEscape(c.type||'Training')}</b></div><div><span>Level</span><b>${htmlEscape(c.level||'Intermediate')}</b></div><div><span>Format</span><b>${htmlEscape(c.format||'Online')}</b></div><div><span>Duration</span><b>${htmlEscape(c.duration||'')}</b></div><div><span>Certificate ID</span><b>${htmlEscape(c.certificate_number)}</b></div></div><div class="content-block"><div class="kicker left">Content & skills</div><div class="skills">${content}</div></div></section>`;
...
async function api(request,env,ctx){
  ...
  if(m==='GET'&&p==='/api/results'){
    if(!userSession(s))return bad('Unauthorized',401);
    const rows=await env.DB.prepare(`SELECT r.*,e.title,e.passing_percentage,c.certificate_number FROM results r JOIN exams e ON e.id=r.exam_id LEFT JOIN certificates c ON c.attempt_id=r.attempt_id AND c.status='valid' WHERE r.user_id=? ORDER BY r.created_at DESC`).bind(s.user_id).all();

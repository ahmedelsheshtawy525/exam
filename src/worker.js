...
async function sendCertificateEmail(env,{to,studentName,certificateTitle,examTitle,verificationUrl,certificateNumber,percentage,issuedAt}={}){
  ...
  const relayToken=String(env.GMAIL_APPS_SCRIPT_TOKEN||'').trim();
  if(!relayUrl)return {sent:false,skipped:true,reason:'GMAIL_APPS_SCRIPT_URL is not configured'};
  if(!relayToken)return {sent:false,skipped:true,reason:'GMAIL_APPS_SCRIPT_TOKEN is not configured'};
  if(!emailOK(to))return {sent:false,skipped:true,reason:'Student email is invalid'};
  ...
  const dateText=issuedAt?new Date(String(issuedAt).includes('T')?issuedAt:issuedAt+'Z').toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'}):new Date().toLocaleDateString('en-US',{year:'numeric',month:'long',day:'numeric'});
  const safeDate=htmlEscape(dateText);
  const subject=`Congratulations, ${studentName||'Student'} — your certificate is ready`;
  const html=`<!doctype html><html><body style="margin:0;background:#f1efe9;font-family:Inter,Arial,sans-serif;color:#0a0a0a"><div style="max-width:700px;margin:0 auto;padding:28px 16px 40px"><div style="background:#050505;border-radius:24px;padding:28px 30px;color:#fff"><div style="display:flex;align-items:center;gap:12px"><img src="${emailOrigin}/ae-logo.png" width="46" height="46" style="object-fit:contain" alt="AE"><div><div style="font-weight:800;font-size:17px">Ahmed Elsheshtawy</div><div style="color:#999;font-size:10px;letter-spacing:.13em;text-transform:uppercase;margin-top:3px">Finance · Assessment · Credentials</div></div></div><div style="height:1px;background:#292929;margin:24px 0"></div><div style="color:#ff4d00;font-size:11px;font-weight:800;letter-spacing:.14em;text-transform:uppercase">Certificate issued successfully</div><h1 style="font-size:34px;line-height:1.08;margin:10px 0 18px;color:#fff">Congratulations, ${safeName}! 🎉</h1><p style="color:#d0d0d0;font-size:15px;line-height:1.75;margin:0 0 14px">You have successfully completed <strong style="color:#fff">${safeExam}</strong> and met the requirements for your certificate.</p><p style="color:#aaa;font-size:14px;line-height:1.7;margin:0">Your official digital credential is now available. You can open it anytime, verify its authenticity online, and share the verification link when needed.</p><div style="margin:26px 0 22px;text-align:center"><a href="${safeUrl}" style="display:inline-block;background:#ff4d00;color:#050505;text-decoration:none;font-weight:900;padding:15px 24px;border-radius:11px;font-size:14px">View Your Certificate ↗</a></div><div style="background:#111;border:1px solid #292929;border-radius:14px;padding:17px 18px"><div style="color:#777;font-size:10px;text-transform:uppercase;letter-spacing:.1em;margin-bottom:10px">Certificate details</div><div style="color:#ddd;font-size:13px;line-height:2"><strong style="color:#fff">Certificate:</strong> ${safeTitle}<br><strong style="color:#fff">Assessment:</strong> ${safeExam}<br><strong style="color:#fff">Score:</strong> ${pct}%<br><strong style="color:#fff">Issued:</strong> ${safeDate}<br><strong style="color:#fff">Certificate ID:</strong> ${safeNo}</div></div></div><div style="padding:24px 8px 0;text-align:center;color:#6f6b64;font-size:12px;line-height:1.7"><p style="margin:0 0 10px">Keep this email for your records. The certificate link above always opens the official verification page.</p><p style="margin:0">If you need to share your credential, simply share the certificate page or its verification link.</p></div><div style="margin-top:26px;border-top:1px solid #d8d3c9;padding:22px 8px 4px;text-align:center"><div style="font-family:'Amsterdam Four_ttf','Brush Script MT',cursive;font-weight:400;color:#111;font-size:24px;line-height:1.1">Ahmed Elsheshtawy</div><div style="color:#777;font-size:11px;margin-top:4px">Finance Assessment Platform · Financial Analysis · Financial Modeling</div><div style="margin-top:12px;font-size:11px"><span style="color:#777">Source:</span> <a href="https://ahmed-portfolio.ahmedelsheshtawyofficial.workers.dev/" style="color:#111;font-weight:700;text-decoration:none">Ahmed Portfolio ↗</a></div><div style="margin-top:12px;color:#999;font-size:10px">© ${new Date().getFullYear()} Ahmed Elsheshtawy. All rights reserved.</div></div></div></body></html>`;
  const text=`Congratulations, ${studentName||'Student'}!\n\nYour certificate has been successfully issued for ${examTitle||'your assessment'} (${pct}%).\n\nCertificate ID: ${certificateNumber||''}\n\nView your certificate: ${verificationUrl||''}`;
  const res=await fetch(relayUrl,{method:'POST',headers:{'Content-Type':'application/json','X-Gmail-Relay-Token':relayToken},body:JSON.stringify({action:'sendCertificateEmail',token:relayToken,to,studentName:studentName||'Student',subject,html,text,certificateNumber,verificationUrl})});
  const responseText=await res.text().catch(()=> '');
  if(!res.ok)throw new Error(`Gmail relay failed (${res.status}): ${responseText.slice(0,500)}`);
  let data=null;try{data=JSON.parse(responseText)}catch{}
  if(!data?.ok||Number(data?.sent)!==1)throw new Error(`Gmail relay rejected the email: ${responseText.slice(0,500)}`);
  return {sent:true,id:data?.messageId||null};
}
async function markCertificateEmail(env,certificateId,result){
  if(!certificateId)return;
  if(result?.sent){await env.DB.prepare("UPDATE certificates SET email_sent_at=CURRENT_TIMESTAMP,email_last_error=NULL,email_attempts=COALESCE(email_attempts,0)+1 WHERE id=?").bind(String(certificateId)).run();}
  else if(result?.skipped){await env.DB.prepare("UPDATE certificates SET email_last_error=?,email_attempts=COALESCE(email_attempts,0)+1 WHERE id=?").bind(clean(result.reason,500),String(certificateId)).run();}
}
async function deliverCertificateEmail(env,certificate,{force=false}={}){
  if(!certificate?.id)return {sent:false,skipped:true,reason:'Certificate not found'};
  if(!force && certificate.email_sent_at)return {sent:false,skipped:true,reason:'Certificate email already sent',alreadySent:true};
  try{
    const result=await sendCertificateEmail(env,certificate);
    await markCertificateEmail(env,certificate.id,result);
    return result;
  }catch(e){
    const message=String(e?.message||e);
    await env.DB.prepare("UPDATE certificates SET email_last_error=?,email_attempts=COALESCE(email_attempts,0)+1 WHERE id=?").bind(clean(message,1000),String(certificate.id)).run();
    throw e;
  }
}
async function getCertificateForEmail(env,certificateId){
  return env.DB.prepare(`SELECT c.id,c.certificate_number,c.email_sent_at,c.email_last_error,c.email_attempts,c.issued_at,c.percentage,c.title AS certificate_title,u.full_name AS student_name,u.email AS to,e.title AS exam_title FROM certificates c JOIN users u ON u.id=c.user_id JOIN exams e ON e.id=c.exam_id WHERE c.id=?`).bind(String(certificateId)).first();
}
async function ensureCertificate(env,request,{attemptId,userId,examId,a,score,total,percentage}){
  const existing=await env.DB.prepare('SELECT id,certificate_number,status,show_answers,email_sent_at,email_last_error,email_attempts,issued_at,title FROM certificates WHERE attempt_id=?').bind(String(attemptId)).first();
  if(existing)return {id:existing.id,issuedAt:existing.issued_at,certificateNumber:existing.certificate_number,verificationUrl:`${new URL(request.url).origin}/verify/${encodeURIComponent(existing.certificate_number)}`,status:existing.status,showAnswers:Number(existing.show_answers)===1,isNew:false,emailSentAt:existing.email_sent_at,emailLastError:existing.email_last_error,emailAttempts:Number(existing.email_attempts||0)};
  ...
  await env.DB.prepare(`INSERT INTO certificates(id,certificate_number,verification_token,attempt_id,user_id,exam_id,score,percentage,title,issued_by,type,level,format,duration,description,skills,issued_at,status,show_answers,email_sent_at,email_last_error,email_attempts) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`)
    .bind(certId,certNo,token,String(attemptId),String(userId),String(examId),score,percentage,
      clean(a.certificate_title||`${a.title} Certificate`,200),clean(a.certificate_issued_by||'Ahmed Elsheshtawy',200),clean(a.certificate_type||'Training',80),clean(a.certificate_level||'Intermediate',80),clean(a.certificate_format||'Online',80),clean(a.certificate_duration||'',80),clean(a.certificate_description||a.description||'',10000),JSON.stringify(skills),issuedAt,'valid',0,null,null,0).run();
  return {id:certId,issuedAt,certificateNumber:certNo,verificationUrl:`${new URL(request.url).origin}/verify/${encodeURIComponent(certNo)}`,status:'valid',showAnswers:false,isNew:true,emailSentAt:null,emailLastError:null,emailAttempts:0};
}
...
function credentialPage(c,request){
  ...
  const qr=`https://api.qrserver.com/v1/create-qr-code/?size=240x240&margin=6&data=${encodeURIComponent(verify)}`;
  ...
  const details=`<section class="details">
    <div class="detail-main">
      <div class="detail-head">
        <div><div class="kicker left">Credential details</div><h2>Verification record</h2></div>
        <span class="status ${status?'':'revoked'}">● ${status?'VALID':'REVOKED'}</span>
      </div>
      <div class="detail-grid">
        <div><span>Type</span><b>${htmlEscape(c.type||'Training')}</b></div>
        <div><span>Level</span><b>${htmlEscape(c.level||'Intermediate')}</b></div>
        <div><span>Format</span><b>${htmlEscape(c.format||'Online')}</b></div>
        <div><span>Duration</span><b>${htmlEscape(c.duration||'')}</b></div>
        <div><span>Issued</span><b>${htmlEscape(issued)}</b></div>
        <div><span>Certificate ID</span><b>${htmlEscape(c.certificate_number)}</b></div>
      </div>
      <div class="description-box">
        <span>Assessment description</span>
        <p>${htmlEscape(description||'This credential recognizes the successful completion of the assessment and demonstrates that the learner met the required criteria.')}</p>
        <div class="description-code">
          <div><span>Certificate ID</span><b id="certificateIdValue">${htmlEscape(c.certificate_number)}</b></div>
          <button type="button" class="copy-description-id" onclick="copyCertificateId()">Copy certificate ID</button>
        </div>
      </div>
      <div class="content-block"><div class="kicker left">Skills</div><div class="skills">${content}</div></div>
    </div>
    <aside class="source-panel">
      <div class="kicker left">Source</div>
      <h3>Credential issuer</h3>
      <div class="source-row"><span>Issued by</span><b>${htmlEscape(c.issued_by||'Ahmed Elsheshtawy')}</b></div>
      <div class="source-row"><span>Assessment</span><b>${htmlEscape(c.exam_title||'Assessment')}</b></div>
      <div class="source-row"><span>Verification</span><b>Official certificate record</b></div>
      <a class="source-link" href="${sourceUrl}" target="_blank" rel="noopener">Open source ↗</a>
    </aside>
  </section>`;
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><meta name="description" content="Verified certificate issued by Ahmed Elsheshtawy"><title>${htmlEscape(c.title)} — ${htmlEscape(c.student_name)}</title><style>
:root{--orange:#ff4d00;--black:#050505;--paper:#f4f2ec;--ink:#0a0a0a;--muted:#77736c;--line:#d7d3c9}
@font-face{font-family:"Amsterdam Four_ttf";src:url("https://db.onlinewebfonts.com/t/07cb29fdcb073fff840edc6de2067b50.woff2") format("woff2"),url("https://db.onlinewebfonts.com/t/07cb29fdcb073fff840edc6de2067b50.ttf") format("truetype");font-weight:400;font-style:normal;font-display:swap}
*{box-sizing:border-box}
html{background:#e9e6dd}
body{margin:0;background:#e9e6dd;color:var(--ink);font-family:Inter,Arial,sans-serif;-webkit-font-smoothing:antialiased}
button,a{font:inherit}
.page{width:min(1120px,100%);margin:0 auto;padding:28px 20px 60px}
.topbar{display:flex;align-items:center;justify-content:space-between;gap:18px;margin-bottom:22px;background:var(--black);padding:12px 14px;border-radius:15px;box-shadow:0 10px 25px rgba(0,0,0,.08)}
.brand{display:flex;align-items:center;gap:9px;color:#fff;min-width:0}.brand img{width:38px;height:38px;object-fit:contain}.brand-name{font-weight:800;font-size:13px;line-height:1.1}.brand-name small{display:block;color:#999;font-size:8px;letter-spacing:.11em;text-transform:uppercase;margin-top:3px}
.actions{display:flex;gap:7px;flex-wrap:wrap;justify-content:flex-end}.btn{border:1px solid #333;background:#111;color:#fff;border-radius:8px;padding:9px 12px;font-weight:800;font-size:10px;cursor:pointer;text-decoration:none;white-space:nowrap}.btn.primary{background:var(--orange);border-color:var(--orange);color:#050505}
.certificate{position:relative;width:100%;aspect-ratio:1.4142857;background:#fbfaf6;border:1px solid #cfc9bc;border-radius:22px;overflow:hidden;box-shadow:0 25px 70px rgba(0,0,0,.12);padding:5.2% 6.1%;display:flex;flex-direction:column;justify-content:center}
.certificate:before{content:"";position:absolute;inset:2.8%;border:1px solid #ded9ce;border-radius:14px;pointer-events:none}.certificate:after{content:"";position:absolute;width:42%;aspect-ratio:1;border:1px solid rgba(255,77,0,.13);border-radius:50%;right:-21%;top:-21%;box-shadow:0 0 0 22px rgba(255,77,0,.025),0 0 0 45px rgba(255,77,0,.018);pointer-events:none}
.cert-logo-wrap{position:relative;z-index:2;display:flex;justify-content:center;align-items:center;margin:0 auto 1.2%}.cert-logo{display:block;width:clamp(24px,4vw,48px);height:clamp(24px,4vw,48px);object-fit:contain}.corner{position:absolute;width:9.5%;height:17%;border:1.5px solid var(--orange);opacity:.72;z-index:2}.corner.tl{left:2.8%;top:4.8%;border-right:0;border-bottom:0;border-radius:9px 0 0 0}.corner.br{right:2.8%;bottom:4.8%;border-left:0;border-top:0;border-radius:0 0 9px 0}
.issuer{text-align:center;font-size:11px;letter-spacing:.18em;text-transform:uppercase;font-weight:800;color:var(--muted)}
.kicker{text-align:center;color:var(--orange);font:800 clamp(7px,1vw,11px) ui-monospace,SFMono-Regular,Menlo,monospace;letter-spacing:.18em;text-transform:uppercase;margin-bottom:1.5%}.kicker.left{text-align:left;margin:0;color:var(--orange)}
.certificate h1{text-align:center;font-size:clamp(27px,5.6vw,66px);line-height:.95;letter-spacing:-.065em;margin:0 auto 2.2%;max-width:820px;position:relative;z-index:1}.rule{width:8%;min-width:45px;height:2px;background:var(--orange);margin:0 auto 2.4%}
.presented{text-align:center;color:var(--muted);font-size:clamp(8px,1.15vw,13px)}.student{text-align:center;font-size:clamp(19px,3.55vw,44px);font-weight:900;letter-spacing:-.045em;margin:.55% 0 1.4%;line-height:1.05;word-break:break-word}.completion{text-align:center;max-width:650px;margin:0 auto;color:#4f4b45;font-size:clamp(8px,1.2vw,15px);line-height:1.55}
.bottom{display:grid;grid-template-columns:minmax(0,1fr) 24% minmax(0,1fr);align-items:end;gap:3%;margin-top:7.2%;position:relative;z-index:1}.signature{text-align:center;min-width:0;color:var(--muted)}.signature-line{position:relative;border-bottom:1px solid #aaa399;height:clamp(24px,3vw,40px);display:flex;align-items:flex-end;justify-content:center;padding:0 8px}.signature b{display:block;font-family:"Amsterdam Four_ttf","Amsterdam Four","Brush Script MT",cursive;font-weight:400;color:var(--ink);font-size:clamp(15px,2vw,29px);line-height:1;white-space:nowrap;transform:translateY(7px)}.signature>span{display:block;font-size:clamp(6px,.75vw,10px);margin-top:8px}.qr{display:flex;flex-direction:column;align-items:center;justify-content:flex-end;text-align:center;min-width:0}.qr img{display:block;width:min(132px,100%);aspect-ratio:1;height:auto;background:#fff;padding:5px;border:1px solid #d8d3c8;border-radius:7px;margin:0 auto 5px}.qr small{display:block;font:700 clamp(6px,.7vw,9px) ui-monospace,SFMono-Regular,Menlo,monospace;color:#777;letter-spacing:.08em;line-height:1.2;text-align:center;width:100%}.meta{text-align:right;font-size:clamp(7px,.8vw,11px);color:var(--muted);line-height:1.55;min-width:0}.meta b{color:var(--ink);font-size:clamp(7px,.85vw,12px);word-break:break-word}.meta .status{margin-top:3px}
.status{display:inline-flex;align-items:center;gap:4px;padding:4px 7px;border-radius:999px;background:#eaf8ef;color:#13733d;font-size:clamp(6px,.7vw,9px);font-weight:800}.status.revoked{background:#fff0ed;color:#a6290f}
.details{margin:34px auto 0;width:100%;padding:32px 34px;background:#f3f0e8;border:1px solid #ded8cc;border-radius:22px;display:grid;grid-template-columns:minmax(0,1fr) 260px;gap:30px;box-shadow:0 12px 30px rgba(0,0,0,.05)}.detail-main{min-width:0}.source-panel{border-left:1px solid #d7d0c4;padding-left:26px}.source-panel h3{margin:5px 0 18px;font-size:18px;letter-spacing:-.03em}.source-row{padding:10px 0;border-top:1px solid #ddd7cb}.source-row:first-of-type{border-top:0}.source-row span{display:block;color:#858078;font-size:10px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px}.source-row b{font-size:12px;color:var(--ink);line-height:1.4}.source-link{display:inline-flex;margin-top:14px;padding:9px 12px;border-radius:9px;background:#111;color:#fff;text-decoration:none;font-size:11px;font-weight:800}
.detail-head{display:flex;align-items:flex-start;justify-content:space-between;gap:18px}.detail-head h2{margin:3px 0 0;font-size:24px;letter-spacing:-.04em}.detail-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:18px}.detail-grid>div{background:#fbfaf6;border:1px solid #ddd7cb;border-radius:11px;padding:11px 13px;min-height:59px}.detail-grid span{display:block;color:#858078;font-size:9px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:5px}.detail-grid b{font-size:11px;color:var(--ink);word-break:break-word}
.description-box{margin-top:16px;padding:14px 16px;background:#fbfaf6;border:1px solid #ddd7cb;border-radius:12px}.description-box>span{display:block;color:#858078;font-size:9px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:7px}.description-box p{margin:0;color:#4f4b45;line-height:1.6;font-size:12px}.description-code{margin-top:12px;padding-top:11px;border-top:1px solid #e0dbd1;display:flex;align-items:center;justify-content:space-between;gap:12px}.description-code>div{min-width:0}.description-code span{display:block;color:#858078;font-size:9px;text-transform:uppercase;letter-spacing:.08em;margin-bottom:4px}.description-code b{display:block;color:var(--ink);font-size:11px;word-break:break-all}.copy-description-id{border:1px solid #ccc5b9;background:#fff;color:#111;border-radius:8px;padding:8px 10px;font-size:10px;font-weight:800;cursor:pointer;white-space:nowrap}.content-block{margin-top:20px}.skills{display:flex;flex-wrap:wrap;gap:7px;margin-top:9px}.skill{display:inline-flex;padding:6px 9px;border-radius:999px;background:#fff;border:1px solid #d7d0c3;font-size:10px;font-weight:800;color:#28251f}.content-empty{color:#77736c;font-size:11px}
.verify-note{margin-top:18px;text-align:center;color:#918d85;font-size:9px}.digital-tools{margin-top:14px;display:flex;justify-content:center;gap:8px;flex-wrap:wrap}.digital-tools a{color:#111;background:#fff;border:1px solid #ccc5b9;border-radius:999px;padding:8px 12px;text-decoration:none;font-size:11px;font-weight:800}.source-block{margin:24px auto 0;max-width:900px;padding:14px 18px;border:1px solid #ded8cc;border-radius:14px;background:#fbfaf6;text-align:center;color:#77736c;font-size:11px}.site-footer{margin:28px auto 0;max-width:900px;border-top:1px solid #d7d3c9;padding:22px 4px 0;text-align:center;color:#77736c}.site-footer-brand{display:flex;justify-content:center;align-items:center;gap:9px}.site-footer-brand img{width:30px;height:30px;object-fit:contain}.site-footer-brand b{color:#111;font-size:13px}.site-footer-role{margin-top:5px;font-size:10px;letter-spacing:.08em;text-transform:uppercase}.site-footer-bottom{margin-top:12px;font-size:10px;color:#999}.site-footer-bottom span{margin:0 4px}.site-footer a{color:#111;font-weight:700;text-decoration:none}
@media(max-width:700px){
  .page{padding:10px 8px 34px}.topbar{padding:8px 9px;border-radius:12px;margin-bottom:10px;gap:8px}.brand{gap:6px}.brand img{width:30px;height:30px}.brand-name{font-size:9px}.brand-name small{font-size:6px;letter-spacing:.08em}.actions{gap:4px}.btn{padding:7px 8px;border-radius:7px;font-size:7px}
  .certificate{border-radius:15px;padding:4.8% 6.2%;box-shadow:0 12px 28px rgba(0,0,0,.1);aspect-ratio:1.44}.cert-logo-wrap{margin-bottom:1%}.cert-logo{width:clamp(22px,6vw,34px);height:clamp(22px,6vw,34px)}.certificate:before{inset:2.6%;border-radius:10px}.corner{width:9%;height:15%}.corner.tl{left:2.8%;top:5%}.corner.br{right:2.8%;bottom:5%}
  .kicker{font-size:6px;margin-bottom:1.2%}.certificate h1{font-size:clamp(22px,7.1vw,42px);margin-bottom:1.9%}.rule{height:1px;min-width:30px;margin-bottom:2%}.presented{font-size:6.5px}.student{font-size:clamp(16px,4.8vw,27px);margin:.5% 0 1.1%}.completion{font-size:6.7px;line-height:1.45;max-width:78%}.bottom{grid-template-columns:minmax(0,1fr) 24% minmax(0,1fr);gap:2.5%;margin-top:6.5%}.signature{font-size:5.5px}.signature-line{height:clamp(20px,6vw,30px);padding:0 3px}.signature b{font-size:clamp(11px,3.7vw,19px);transform:translateY(5px)}.signature>span{font-size:5px;margin-top:6px}.qr img{width:100%;max-width:92px;padding:3px;border-radius:5px;margin-bottom:3px}.qr small{font-size:5px}.meta{font-size:5.5px;line-height:1.45}.meta b{font-size:6px}.status{font-size:5px;padding:2px 4px;gap:2px}
  .details{margin-top:12px;padding:16px 14px;border-radius:15px;grid-template-columns:1fr;gap:16px}.detail-head{gap:8px}.detail-head h2{font-size:18px}.detail-grid{grid-template-columns:repeat(2,1fr);gap:7px;margin-top:12px}.detail-grid>div{padding:9px 10px;min-height:50px;border-radius:9px}.detail-grid span{font-size:7px}.detail-grid b{font-size:9px}.description-box{margin-top:11px;padding:11px 12px;border-radius:10px}.description-box>span,.description-code span{font-size:7px}.description-box p{font-size:9px;line-height:1.55}.description-code{margin-top:9px;padding-top:8px;gap:7px}.description-code b{font-size:8px}.copy-description-id{padding:6px 8px;font-size:8px}.content-block{margin-top:13px}.skills{gap:5px;margin-top:7px}.skill{padding:5px 7px;font-size:8px}.source-panel{border-left:0;border-top:1px solid #d7d0c4;padding-left:0;padding-top:15px}.source-panel h3{font-size:16px;margin:4px 0 10px}.source-row{padding:8px 0}.source-row span{font-size:8px}.source-row b{font-size:10px}.source-link{margin-top:10px;font-size:9px;padding:7px 9px}.digital-tools{margin-top:10px}.digital-tools a{font-size:9px;padding:7px 10px}.verify-note{font-size:8px}.site-footer{margin-top:18px;padding-top:15px}.site-footer-brand img{width:24px;height:24px}.site-footer-brand b{font-size:11px}.site-footer-role{font-size:8px}.site-footer-bottom{font-size:8px}
}
@media(max-width:380px){.topbar .brand-name{display:none}.btn{font-size:6.5px;padding:6px 7px}.certificate h1{font-size:21px}.student{font-size:15px}.completion{font-size:6px}.qr img{max-width:78px}.meta{font-size:5px}.meta b{font-size:5.5px}}
@media print{
  @page{size:A4 landscape;margin:0}
  html,body{width:297mm;height:210mm;background:#fff!important}
  body{-webkit-print-color-adjust:exact;print-color-adjust:exact}
  .page{width:297mm;max-width:none;height:210mm;margin:0;padding:0}
  .topbar,.digital-tools,.verify-note,.details,.source-block,.site-footer{display:none!important}
  .certificate{width:297mm;height:210mm;aspect-ratio:auto;min-height:0;border:0;border-radius:0;box-shadow:none;padding:11mm 18mm}
  .certificate:before{inset:5.5mm;border-radius:0}.corner{width:25mm;height:32mm}.corner.tl{left:8mm;top:10mm}.corner.br{right:8mm;bottom:10mm}
  .cert-logo{width:14mm;height:14mm}.cert-logo-wrap{margin-bottom:2mm}.certificate h1{font-size:18mm}.student{font-size:11mm}.completion{font-size:4mm}.kicker{font-size:2.5mm}.presented{font-size:3.5mm}.rule{height:.6mm}.signature-line{height:13mm}.signature b{font-size:6mm;transform:translateY(2mm)}.signature>span{font-size:2.5mm;margin-top:3mm}.signature{font-size:3mm}.qr img{width:34mm;height:34mm;max-width:none}.qr small{font-size:2.3mm}.meta{font-size:3mm}.meta b{font-size:3.1mm}.status{font-size:2.5mm;padding:1.5mm 2.5mm}
}
</style></head><body><main class="page"><header class="topbar"><div class="brand"><img src="/ae-logo.png" alt="Ahmed Elsheshtawy"><div class="brand-name">Ahmed Elsheshtawy<small>Finance · Assessment · Credentials</small></div></div><div class="actions"><button class="btn" onclick="window.print()">Print Certificate</button><button class="btn primary" onclick="copyLink()">Copy Verification Link</button></div></header><section class="certificate"><div class="corner tl"></div><div class="corner br"></div><div class="cert-logo-wrap"><img class="cert-logo" src="/ae-logo.png" alt="Ahmed Elsheshtawy logo"></div><div class="kicker">Certificate of Achievement</div><h1>${htmlEscape(c.title)}</h1><div class="rule"></div><div class="presented">This certificate is presented to</div><div class="student">${htmlEscape(c.student_name)}</div><div class="completion">For successfully completing this assessment and meeting the required criteria.</div><div class="bottom"><div class="signature"><div class="signature-line"><b>${htmlEscape(c.issued_by||'Ahmed Elsheshtawy')}</b></div><span>Ahmed Elsheshtawy</span></div><div class="qr"><img src="${qr}" alt="QR code for certificate verification"><small>SCAN TO VERIFY</small></div><div class="meta"><div>Issue date<br><b>${htmlEscape(issued)}</b></div><div>Certificate ID<br><b>${htmlEscape(c.certificate_number)}</b></div><div style="margin-top:5px"><span class="status ${status?'':'revoked'}">● ${status?'VALID':'REVOKED'}</span></div></div></div></section>${details}<div class="digital-tools"><a href="${htmlEscape(assessment)}">View Assessment ↗</a><a href="${htmlEscape(verify)}">Refresh verification ↗</a></div><div class="verify-note">The QR code and certificate link open the official verification page.</div><footer class="site-footer"><div class="site-footer-brand"><img src="/ae-logo.png" alt="AE"><b>Ahmed Elsheshtawy</b></div><div class="site-footer-role">Finance Assessment Platform · Financial Analysis · Financial Modeling</div><div class="site-footer-bottom">© ${new Date().getFullYear()} Ahmed Elsheshtawy. All rights reserved.<span>·</span>Built with intention.</div></footer></main><script>const verifyUrl=${JSON.stringify(verify)};const certificateId=${JSON.stringify(String(c.certificate_number||''))};async function copyText(value,success){try{await navigator.clipboard.writeText(value);alert(success)}catch(e){prompt('Copy:',value)}}async function copyLink(){copyText(verifyUrl,'Verification link copied.')}async function copyCertificateId(){copyText(certificateId,'Certificate ID copied.')} </script></body></html>`;
}
...
async function api(request,env,ctx){
        ...
        if(existing){
          ...
          if(Number(existing.passed)===1 && Number(a.certificate_enabled??1)===1){
            try{
              ...
              if(certificate?.isNew){const emailTask=deliverCertificateEmail(env,{...certificate,to:s.email||a.email,studentName:s.full_name||a.full_name,certificateTitle:a.certificate_title||`${a.title} Certificate`,examTitle:a.title,verificationUrl:certificate.verificationUrl,certificateNumber:certificate.certificateNumber,percentage:Number(existing.percentage),issuedAt:certificate.issuedAt||new Date().toISOString()}).catch(e=>console.error('CERTIFICATE EMAIL ERROR:',e?.message||e));if(ctx?.waitUntil)ctx.waitUntil(emailTask);else await emailTask;}
            }catch(certError){
  ...
  if(adminSession(s)){
    ...
    if(m==='DELETE'&&p.match(/^\/api\/admin\/students\/\d+\/?$/)){
      const auth=adminOnly(s);if(auth)return auth;
      const id=idNum(p.split('/')[4]);if(!id)return bad('Invalid student');
      const student=await env.DB.prepare('SELECT id,full_name,student_id FROM users WHERE id=?').bind(id).first();if(!student)return bad('Student not found',404);
      await env.DB.batch([
        ...
        env.DB.prepare('DELETE FROM exam_attempts WHERE user_id=?').bind(id),
        env.DB.prepare('DELETE FROM users WHERE id=?').bind(id)
      ]);
      return json({ok:true,deletedStudentId:id});
    }
    ...
    if(m==='POST'&&p.match(/^\/api\/admin\/certificates\/[^/]+\/send-email$/)){
      ...
      const c=await getCertificateForEmail(env,id);if(!c)return bad('Certificate not found',404);
      try{const result=await deliverCertificateEmail(env,{...c,verificationUrl:`${new URL(request.url).origin}/verify/${encodeURIComponent(c.certificate_number)}`,force:true});return json({ok:true,...result,emailSentAt:(await getCertificateForEmail(env,id))?.email_sent_at||null});}
      catch(e){return bad(String(e?.message||e),502)}
    }
...
export default {async fetch(request,env,ctx){try{
  ...
  if(path.match(/^\/exam-review\/\d+\/?$/)||path.match(/^\/certificate-exam\/[^/]+\/?$/))return env.ASSETS.fetch(new Request(new URL('/index.html',request.url),request));
